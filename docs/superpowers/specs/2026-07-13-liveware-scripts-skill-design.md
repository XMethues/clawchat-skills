...
2. Skill 能对目标 Hermes skill 生成固定路径的两个脚本。
3. 生成过程从 `SKILL.md` 提取稳定名称和展示名称。
4. Skill 不改变用户提供的 server 形态。
5. 状态按 skill 隔离在 `$HOME/.clawling/apps`。
6. setup 登录、应用恢复、创建、状态写入和 ClawChat 注册行为幂等且安全。
7. start 沿用项目 server 接口，并只在服务可用后绑定 tunnel。
8. 检查器能发现当前 Office 和 Tarot 脚本中的关键协议差异。
9. 修复不会改动目标 server 实现或正确的生命周期管理。
10. 没有真实环境时只报告静态检查结果，不声称运行验证通过。
