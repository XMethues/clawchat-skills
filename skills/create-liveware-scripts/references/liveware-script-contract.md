...
Lifecycle wrappers are part of exact argv. If the confirmed server launch uses a wrapper such as `nohup`, keep it as a command element rather than treating ordinary backgrounding as equivalent, and include each bare wrapper or runtime in `required_commands`.
